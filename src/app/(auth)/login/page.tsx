  ...
  return (
    <div className="w-full bg-white rounded-[2rem] sm:rounded-[2.5rem] md:rounded-[3rem] overflow-hidden">
      ...
      <div className="px-4 sm:px-6 md:px-8 pb-4 sm:pb-6 md:pb-8">
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-center mb-4 sm:mb-5 md:mb-6">Login</h1>
        <form onSubmit={onSubmit} className="space-y-3 sm:space-y-4">
          {/* Email/Phone Field */}
          <div className="space-y-1.5 sm:space-y-2">
            <label className="block text-xs sm:text-sm">Email / Phone</label>
            <input
              type="text"
              value={username}
              onChange={(e) => setUsername(e.target.value)}
              placeholder="Enter your phone / email here..."
              className="w-full h-[35px] sm:h-[40px] md:h-[45px] px-3 sm:px-4 text-sm sm:text-base bg-[#F6F6F6] hover:bg-gray-100 rounded-full border-none focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
          ...
          <div className="relative py-2 sm:py-3">
            ...
            <div className="relative flex justify-center">
              <span className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 flex items-center justify-center text-xs sm:text-sm text-gray-500 bg-[#F6F6F6] rounded-full">OR</span>
            </div>
          </div>
