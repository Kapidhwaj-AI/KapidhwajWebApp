    ...
    <div className="space-y-1.5  sm:space-y-2">
      {isPasswordField && showForgotPasswordLabel ? (
        <div className="flex justify-between items-center">
          {label && <label htmlFor={`${label.toLowerCase()}`} className="block text-xs sm:text-sm text-black">
            {label}
        ...
        label && <label className="block text-xs sm:text-sm text-black">
          {label}
          {required && <span className="text-red-500 ml-1">*</span>}
        </label>
      )}
      <div className="relative">
        ...
        <input id={isOtp ? `otp-${index}` :`${label?.toLowerCase()}`}
          ...
          className="w-full h-[35px] sm:h-[40px] md:h-[45px] px-3 sm:px-4 text-sm sm:text-base bg-[#F6F6F6] text-black hover:bg-gray-100 rounded-lg border-none focus:outline-none focus:ring-2 focus:ring-blue-500"
        />
