        ...
        <div className="w-full bg-[var(--surface-200)] rounded-4xl shadow-lg overflow-hidden">
            <div className="flex justify-between md:flex-row flex-col md:gap-0 gap-1 items-start md:items-center p-2 md:px-4 md:pt-4">
                <div className="flex items-center gap-1 md:gap-3">
                    {alert.alertType === 'FACE_DETECTION' && alert?.persons && <Link href={{ pathname: "/person-details", query: { id: alert.person_ids[0] } }}><img className="rounded-full object-cover w-10 h-10 aspect-auto" src={BASE_URL + `/${alert?.persons[0].gcp_image_path}`} /></Link>}
            ...
            >
                {alert?.frame_url && <img src={BASE_URL + alert?.frame_url} alt={alert?.alertType} width={1000} height={1000} className="object-cover w-auto h-auto rounded-2xl" />}
            ...
            {isPreview && <AlertPreviewDialogue onClose={() => setIsPreview(false)} imageUrl={BASE_URL + alert.frame_url} alertType={alert.alertType} />}
        </div>
    );
}