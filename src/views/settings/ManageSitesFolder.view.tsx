        ...
        <div className='grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-7 px-2 md:px-4 pb-3 h-full w-full'>
            <div className='bg-[var(--surface-100)] scrollbar-hide rounded-2xl md:rounded-4xl flex flex-col  md:p-6 p-3 h-full max-h-[84vh]'>
                ...
                <div className='flex-1 flex flex-col gap-3 overscroll-y-auto py-4'>
                    {isSiteLoading ? <Spinner /> : sites.length === 0 ? <div className='text-center h-full flex items-center justify-center'>{t('no_data_found')}</div> : (
                        sites.map((site, index) => (
                            <ListCard setIsEdit={setIsEdit} setName={setName} setIsOpen={setIsOpen} setId={setId} selectedId={orgId} key={index} setIsOrg={setIsOrg} setIsDelete={setIsDelete} handleNavigate={handleNavigate} data={site} />
                        )))
                    }
                </div>
